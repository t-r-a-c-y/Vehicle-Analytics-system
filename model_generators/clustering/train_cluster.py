...
SEGMENT_FEATURES = ["estimated_income", "selling_price", "kilometers_driven"]  # Added feature for better clustering
...
cluster_mapping = {
    ...
    sorted_clusters[3]: "Luxury",  # Added for 4 clusters
}
...
silhouette_avg = round(silhouette_score(X_scaled, df["cluster_id"]), 2)  # Aim >0.9; test on data
...
def evaluate_clustering_model():
    return {
        ...
        "cv_table": cv_df.to_html(classes="table table-bordered table-striped table-sm", float_format="%.2f", justify="center", index=False),  # For exercise
    }